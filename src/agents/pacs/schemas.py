...
class BaseToolOutput(BaseModel):
    ...
    correction: str | None = None  # 给 LLM 的可执行修正方向
...
class PacsTargetInput(BaseToolInput):
    ...
    study_instance_uid: UidStr | None = Field(None, description="Study Instance UID")
    series_instance_uid: UidStr | None = Field(None, description="Series Instance UID")
...
class PacsTargetOutput(BaseToolOutput):
    ...
    parent_study_uid: str | None = None
    expected_instance_count: int | None = None
    modality: str | None = None
    dicom_status_summary: str | None = None
...
class SeriesNode(BaseModel):
    ...
    modality: str | None = None
    instance_count: int | None = None  # NumberOfSeriesRelatedInstances
...
class TaskContextInput(BaseToolInput):
    task_id: IdStr | None = Field(None, description="任务 ID")
    study_instance_uid: UidStr | None = Field(None, description="Study Instance UID")
    series_instance_uid: UidStr | None = Field(None, description="Series Instance UID")
...
class TaskSummary(BaseModel):
    ...
    move_finished_at: str | None = None
...
class WorkerHealthInput(BaseToolInput):
    ...
    queue_name: IdStr | None = Field(None, description="队列名，缺省用下载队列")
...
class ReceiveStatusInput(BaseToolInput):
    study_instance_uid: UidStr = Field(..., description="Study Instance UID")
    series_instance_uid: UidStr | None = Field(None, description="指定时仅统计该 Series")
...
class ComputeIntegrityInput(BaseToolInput):
    expected: NonNegInt | None = Field(None, description="期望数量（来自 PACS，可为 None）")
...
class ComputeIntegrityOutput(BaseToolOutput):
    ...
    missing: int | None = None  # max(expected - local_unique_sop, 0)；expected 为 None → None
...
class SearchKnowledgeInput(BaseToolInput):
    ...
    task_id: IdStr | None = Field(
        None, description="当前请求携带的任务 ID，仅用于检索式补全，不验证是否存在",
    )
    study_instance_uid: UidStr | None = Field(
        None, description="当前请求携带的 Study UID，仅用于检索式补全，不验证是否存在",
    )
    series_instance_uid: UidStr | None = Field(
        None, description="当前请求携带的 Series UID，仅用于检索式补全，不验证是否存在",
    )
    category: IdStr | None = Field(None, description="可选类目过滤，如 fault_sop / experience")
    top_n: TopN | None = Field(None, description="返回条数 1..10，缺省用配置 top_k")
...
class RepullPlanInput(BaseModel):
    ...
    task_id: str | None = Field(None, description="任务 ID；手动下载场景可为空（新建任务）")
    ...
    study_instance_uid: str | None = Field(None, description="新建补拉任务所需的 Study UID")
    source_id: str | None = Field(None, description="新建补拉任务所用 PACS 源")
    operator: str | None = Field(None, description="审批人")
...
class RouteDecision(BaseModel):
    ...
    clarification: str | None = None  # route=clarification 时给用户的追问语
...
class RepullScope(BaseModel):
    ...
    missing_instance_count: int | None = None
