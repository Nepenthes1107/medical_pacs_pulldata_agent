...
from src.agents.pacs.state import AgentState, truncate_evidence

# act 只能执行注册表内工具（护栏 3/4）。注册表与绑定给模型的工具是同一份，
# 不再单独维护执行表——两份表分叉时「模型可见」与「工程允许」会不一致。
from src.agents.pacs.tools import TOOL_REGISTRY
...
def reason(state: AgentState) -> dict:
    ...
    """
    from src.core.llm import get_reasoning_model, llm_available, reasoning_prompt
...
def _reason_degraded(state: AgentState) -> dict:
    ...
    if "query_task_context" not in tr and (study or state.get("task_id")):
        ...
        if state.get("task_id"):
            task_args["task_id"] = state["task_id"] or ""
        if study:
            task_args["study_instance_uid"] = study
        return _pending_with_message([{"name": "query_task_context", "args": task_args}])
        if study:
...
def _run_tool_call(call: dict) -> dict:
    ...
    # 注册表内工具均绑定 Pydantic args_schema；非 BaseModel（dict 等）属配置异常，拒绝执行。
    if not isinstance(schema, type) or not issubclass(schema, BaseModel):
        return _rejected("%s has no pydantic args_schema" % name, "invalid_tool_schema",
                         "该工具配置异常，不能执行")
    try:
    ...
    func = tool.func
    if func is None:  # 同步工具必有实现；None 只出现在配置缺失时，护栏式拒绝。
        return _rejected("%s has no callable implementation" % name, "invalid_tool_schema",
                         "该工具配置异常，不能执行")
    try:
    ...
    from src.agents.pacs.schemas import BaseToolOutput
...
def act(state: AgentState) -> dict:
    ...
    for call in calls:
        history.append(_fingerprint(call.get("name") or "", call.get("args", {}) or {}))
    ...
    # 无法审计、也无法在窗口外校验的诊断结果。
    from src.infrastructure.db.audit import record_tool_evidence
...
def diagnose(state: AgentState) -> dict:
    ...
    try:
        from src.agents.pacs.prompts import diagnosis_few_shot, get_system_prompt, xml_blocks
        from src.agents.pacs.schemas import GroundedDiagnosis
        from src.core.llm import get_structured_model, reasoning_prompt
...
def verify_diagnosis(state: AgentState) -> dict:
    ...
    """
    from src.agents.pacs.harness import filter_grounded_claims, verify_citations
...
def reflect(state: AgentState) -> dict:
    ...
    try:
        from src.agents.pacs.harness import build_citation_facts
        from src.agents.pacs.prompts import get_system_prompt, xml_blocks
        from src.agents.pacs.schemas import Reflection
        from src.core.llm import contextual_prompt, get_structured_model
        ...
        if r.inference_status == "overstated":
            diagnosis["confidence"] = {
                ...
                "high": "uncertain",
                "uncertain": "uncertain",
            }.get(diagnosis.get("confidence") or "uncertain", "uncertain")
...
def plan_repull(state: AgentState) -> dict:
    ...
    try:
        from src.agents.pacs.prompts import get_system_prompt, xml_blocks
        from src.agents.pacs.schemas import RepullPlan
        from src.core.llm import contextual_prompt, get_structured_model
        ...
        from src.agents.pacs.harness import verify_citations
