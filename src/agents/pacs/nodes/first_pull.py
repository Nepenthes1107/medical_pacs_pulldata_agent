...
故不进 reason↔act 循环，直接复用 src.application.pull_service.create_pull_task（与 /tasks/pull 同一份逻辑）。
首拉视为常规业务动作、不需人工审批（护栏 4 仍只管补拉等诊断结论触发的写操作）。
"""
import logging

from src.agents.pacs.state import AgentState
from src.application import pull as pull_service
from src.schema.pacs import DataLevel, PullTaskRequest
...
def first_pull(state: AgentState) -> dict:
    ...
    try:
        task_id, status = pull_service.create_first_pull(request)
