...
from pynetdicom.sop_class import (  # type: ignore[attr-defined]  # pynetdicom 3.0.4 动态导出，stub 不含
...
from src.schema.pacs import InstanceResult, MoveResult, PacsEchoResult, SeriesResult, StudyResult
...
def _dataset_text(dataset: Dataset, key: str) -> str | None:
...
def _dataset_int(dataset: Dataset, key: str) -> int | None:
    value = getattr(dataset, key, None)
    if value in (None, ""):
        return None
    try:
