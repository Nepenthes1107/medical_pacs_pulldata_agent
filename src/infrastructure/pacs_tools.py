...
from src.agents.pacs.schemas import (
...
from src.infrastructure.db.repositories import ActionAuditRepository, TaskRepository
from src.infrastructure.db.session import session_scope
from src.infrastructure.idempotency import idempotency_lock
from src.infrastructure.messaging import inspect_queue, publisher
from src.infrastructure.pacs.client import PacsClient
from src.infrastructure.pacs.scanner import scan_local_dicom
from src.infrastructure.rag import search_knowledge
...
def query_pacs_target(
    ...
    try:
        if series_instance_uid:
            ...
            study_match = next(
                (s for s in studies if s.study_instance_uid == study_instance_uid), None
            )
            out.study_exists = study_match is not None
            if study_match:
                out.expected_instance_count = study_match.number_of_study_related_instances
                out.modality = study_match.modality
        return out
...
def query_receive_status(
    ...
    try:
        with session_scope() as db:
            rows = TaskRepository(db).received_images(study_instance_uid, series_instance_uid)
...
def query_worker_health(queue_name: str | None = None) -> WorkerHealthOutput:
    """查询下载队列的 message/consumer 数，合成 Worker 存活/积压一体信号。"""
    name = queue_name or settings.rabbitmq.download_queue
    try:
        queue = inspect_queue(name)
        msg_count = int(queue["message_count"])
        consumer_count = int(queue["consumer_count"])
...
def scope_fingerprint(missing_targets: list[dict] | None) -> str:
...
def execute_repull_plan(
    ...
    idempotency_key: str = "",
    missing_targets: list[dict] | None = None,
    study_instance_uid: str | None = None,
    ...
    try:
        acquired = idempotency_lock.acquire(idempotency_key, run_id)
...
def _load_task_brief(task_id: str) -> dict | None:
...
def _create_repull_task(run_id, origin_task_id, strategy, idempotency_key, targets,
    ...
    复用 src.application.pull_service.create_pull_task（建任务 + 投队列的单一真相），不重写业务。
    """
    from src.agents.pacs.schemas import RepullExecutionOutput
    from src.application import pull_service
    from src.core.enums import DataLevel
    from src.schema.pacs import PullTaskRequest, SeriesPullItem
...
def _submit_repull(run_id, task_id, strategy, idempotency_key, operator, force):
    ...
    from src.agents.pacs.schemas import RepullExecutionOutput
    from src.core.enums import ArchiveStatus, DataLevel
    ...
    try:
        publisher.download_task(message)
    except Exception as exc:
...
def _audit(db, run_id, task_id, operator, idempotency_key, result, action, detail):
    ...
    audit = ActionAuditRepository(db).require_by_key(idempotency_key)
    audit.run_id = run_id
...
def _claim_action(run_id, task_id, operator, idempotency_key, strategy, targets) -> bool:
    ...
    from src.infrastructure.db.models import AgentActionAudit
...
def _mark_action_failed(idempotency_key: str, error: str) -> None:
    ...
    with session_scope() as db:
        audit = ActionAuditRepository(db).get_by_key(idempotency_key)
        if audit:
            audit.result = "failed"
