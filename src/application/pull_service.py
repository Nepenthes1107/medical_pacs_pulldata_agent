...
from src.infrastructure.db.models import ArchiveModel, DownloadTask, SeriesModel, StudyModel
...
def _clear_abort_flags(request: PullTaskRequest, series_list: list[dict],
    ...
    uids: list[str] = [
        uid for i in series_list
        if (uid := i.get("series_instance_uid"))
    ]
...
def _first_series_uid(series_list: list[dict]) -> str | None:
    return series_list[0].get("series_instance_uid") if series_list else None


def _series_exists(series_list: list[SeriesResult], series_uid: str | None) -> bool:
    return bool(series_uid) and any(i.series_instance_uid == series_uid for i in series_list)


def _task_modality(series_list: list[SeriesResult], series_uid: str | None) -> str | None:
    if series_uid:
        for item in series_list:
...
def _upsert_study(db: Session, request: PullTaskRequest, expected_count: int | None) -> None:
...
def _build_data_id(level: str, study_uid: str, series_uid: str | None,
                   task_id: str | None = None, series_list: list[dict] | None = None) -> str:
...
def refresh_task_expected(task: DownloadTask, db: Session) -> int | None:
...
def _recompute_expected(task: DownloadTask, study_result: StudyResult | None,
                        pacs_series: list[SeriesResult]) -> int | None:
